...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=32, nc=3, f_filt=4):
        ...
        self.fc_mu = nn.Linear(1024 * 14 * 14, z_dim)  # B, z_dim
        self.fc_logvar = nn.Linear(1024 * 14 * 14, z_dim)  # B, z_dim
        self.decoder = nn.Sequential(
            nn.Linear(z_dim, 1024 * 28 * 28),  # B, 1024*8*8
            View((-1, 1024, 28, 28)),  # B, 1024,  8,  8
...
class CVAE(nn.Module):  # My Module
    """Encoder-Decoder architecture for both WAE-MMD and WAE-GAN."""

    def __init__(self, z_dim=32, nc=3, f_filt=4, n_class=10):
    ...
    def forward(self, x):
        z = self._encode(x)
        mu, logvar = self.fc_mu(z), self.fc_logvar(z)
        cls_soft = self.classifier(z)
        _, cls_hard = torch.max(cls_soft, 1)
        cls_emb = self.label_emb(cls_hard)
        z = self.reparameterize(mu, logvar)
        # print('cls',cls.shape)
        # print('z',z.shape)
        x_recon = self._decode(torch.cat((cls_emb, z), dim=-1))

        return x_recon, z, mu, logvar, cls_soft
...
class C_Discriminator(nn.Module):
    ...
    def __init__(self, z_dim=10, n_class=10):
